...
export const firstCharCapital = (str: string): string => upper(str.substr(0, 1)) + lower(str.substr(1));
...
export const iotaUtilFuncs = (iotaReturn: IotaReturn): IotaUtilFunctions => {
    return Object.entries(iotaReturn).reduce((utilFuncs, [key, val]) => {
        if (typeof parseInt(key) !== "number" && typeof val === "number") {
            ...
            utilFuncs[`has{funcName}`] = (stateByte: number): boolean => (stateByte & val) === val;
            utilFuncs[`toggle${funcName}`] = (stateByte: number): number => stateByte ^ val; 
            utilFuncs[`on{funcName}`] = (stateByte: number): number => stateByte | val; 
            utilFuncs[`off{funcName}`] = (stateByte: number): number => utilFuncs[`has${funcName}`](stateByte) ? utilFuncs[`toggle${funcName}`](stateByte) : stateByte;
        }
        return utilFuncs;
    }, {})
}
